...
module.exports = {
  ...
  getProductsByStores: async (root, { storesIDs, take, skip }) => {
    try {
      const db = await connectDB();
      let date = new Date()
      date = new Date().setDate(date.getDate() - date.getDay() + 1)
      consoleFile.log({ storesIDs, take, skip })
  ...
  getProductsByKeywordAndStore: async (root, { keyword, storesIDs, take, skip }) => {
    try {
      const db = await connectDB();
      consoleFile.log({ keyword, storesIDs, take, skip })
      let date = new Date()
      date = new Date().setDate(date.getDate() - date.getDay() + 1)
