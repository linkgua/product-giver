...
module.exports = {
  ...
  getProductsWords: async (root, { keywords, storesIDs }) => {
    try {
      const db = await connectDB();
      let date = new Date()
      date = new Date().setDate(date.getDate() - date.getDay() - 180)
      let promises = keywords.map(async word => {
        if(storesIDs.length){
      ...
      let results = await Promise.all(promises).then(productsArray => {
        return productsArray.map((products,index) => {
          if(products.length){
